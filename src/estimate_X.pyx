...
def estimate_X():
    for m in range(state.M):
        ...
        # Update XFm
        B1 = state.phiFm[m] * state.RFm[m] * state.invCovMatFm[m]
        A1B1 = state.phiFm[m] * state.invCovMatFm[m].dot(state.sumXFmr[m])
        
        B2 = state.invCovMatF * state.tauF * tmpV
        ...
        B1 = state.phiHm[m] * state.RHm[m] * state.invCovMatHm[m]
        A1B1 = state.phiHm[m] * state.invCovMatHm[m].dot(state.sumXHmr[m])
        
        B2 = state.invCovMatH * state.tauH * tmpV
        A2 = state.muH.copy()
