...
def estimate_phi_m_withoutSpatial():
    ...
    for m in range(state.M):
        ...
        rate = np.sum(state.XHmrDiff[m,0:state.RHm[m]]*state.XHmrDiff[m,0:state.RHm[m]])
        ...
        rate = np.sum(state.XFmrDiff[m,0:state.RFm[m]]*state.XFmrDiff[m,0:state.RFm[m]])
